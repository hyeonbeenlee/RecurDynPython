...
def CreateSplineExpressions(Spline_Dir):
    ...
    for idx_file, file_spline in enumerate(FileList):
        ...
        if number_sensor in Target and name_file[0] == "A":
            ...
            model.CreateExpression(
                f"{name_expression}", f"AKISPL(TIME,0,{name_spline},0)"
            )
    ...
    # Accelerations
    model.CreateExpressionWithArguments(
        "Ex_ATX_CM", "ACCX(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_ATY_CM", "ACCY(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_ATZ_CM", "ACCZ(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_ARX_CM", "WDTX(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_ARY_CM", "WDTY(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_ARZ_CM", "WDTZ(1)", ["Cask.Marker_CaskCradleCM"]
    )

    # Velocities
    model.CreateExpressionWithArguments(
        "Ex_VTX_CM", "VX(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_VTY_CM", "VY(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_VTZ_CM", "VZ(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_VRX_CM", "WX(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_VRY_CM", "WY(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_VRZ_CM", "WZ(1)", ["Cask.Marker_CaskCradleCM"]
    )

    # Displacements
    model.CreateExpressionWithArguments(
        "Ex_DTX_CM", "DX(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_DTY_CM", "DY(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_DTZ_CM", "DZ(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_DRX_CM", "ROLL(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_DRY_CM", "PITCH(1)", ["Cask.Marker_CaskCradleCM"]
    )
    model.CreateExpressionWithArguments(
        "Ex_DRZ_CM", "YAW(1)", ["Cask.Marker_CaskCradleCM"]
    )
...
def ExportSensorDisplacements():
    ...
    for number in SensorNum:
        ...
        model.CreateRequestStandard(
            f"Rq_Sensor_{number}",
            RequestStandardType.Displacement,
            SensorMarker,
            TrailerMarker,
            TrailerMarker,
        )
...
def CreateExpressionLoop():
    ...
    rdynList = []
    DataDirList = os.walk(
        "D:\Research\Trailer2022\Sea_Tuning\Model_OtherScenarios2\\2. Sea"
    )
    for root, subdirs, files in DataDirList:
        if "Detrended" in root:
            # Open base rdyn
            model_document = application.OpenModelDocument(
                "D:\Research\Trailer2022\Sea_Tuning\Model_OtherScenarios2\CaskCraddle_LMM_Blank.rdyn"
            )
            model = model_document.Model
            # Case name
    ...
    for rdyn in rdynList:
        ...
        model_document.ModelProperty.DynamicAnalysisProperty.SimulationStep.Value = (
            IParametricValue(model.GetEntity("PV_NumSteps")).Value
        )
        model_document.ModelProperty.DynamicAnalysisProperty.SimulationTime.Value = (
            IParametricValue(model.GetEntity("PV_EndTime")).Value
        )
...
def CreateGRoadWith1DSplineData():
    ...
    for CaseNo in CaseNums:
        ...
        Profiles = os.walk(
            f"C:\\Users\LHB-MSLab\Documents\GitHub\KAERI-2022\RoadTest_2022\Case0{CaseNo}"
        )
        for dir, subdirs, files in Profiles:
            if files and "R=" in dir and "ModGeo" in dir:
                ...
                for file in files:
                    if "csv" in file:
                        RoadOutline = ground.CreateOutlineGeometryWithFile(
                            f"Outline_Uneven{idx_file + 1}_{Roughness}",
                            f"{dir}\\{file}",
                        )
                        multiCurve.append(RoadOutline)
                        print(
                            f"Outline_Uneven{idx_file + 1}_{Roughness} from {dir}\\{file}"
                        )
                        idx_file += 1

                print(f"Surface_Uneven_{Roughness}")
                RoadProfileSheet = IGeometrySheet(
                    ground.CreateSplineSurfaceGeometry(
                        f"Surface_Uneven_{Roughness}", multiCurve
                    )
                )
                ...
                for i in range(NumFaces):
                    FaceList.append(
                        ground.GetEntity(
                            f"Surface_Uneven_{Roughness}.Face{NumFaces - i}"
                        )
                    )
                ...
                ground.CreateGRoadWithFace(
                    f"GRoad_Uneven_{Roughness}",
                    FaceList,
                    roadRefFrame,
                    f"GRoad_Uneven_Case0{CaseNo}_{Roughness}.rdf",
                )


def TireParamScaler(
    modelPath,
    TireFileName="UATire_MMKS_Trailer_Comb_Bump",
    radialK=1,
    longtitudinalK=1,
    lateralK=1,
    camberK=1,
...
def CreateSensor20RelativeDisplacements():
    ...
    for idx_case, c in enumerate(CaseNo):
        ...
        if refFrame_init == None:
            ...
            initialpoint.SetOrigin(-5729.0, 1436.44951453657, 2697.7828)
        ...
        for dir in direction:
            model.CreateExpressionWithArguments(
                f"Ex_{dir}_Sensor_20",
                f"{dir}(1,2)",
                ["Sensor_20.CM", "Trailer_6.Initial"],
            )
        model_document.FileSave(
            f"D:\Research\Trailer2022\Ground_Model\\Trailer_{c}.rdyn", True
        )
